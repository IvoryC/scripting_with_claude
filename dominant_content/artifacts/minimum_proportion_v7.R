...
# Count non-NA values per sample in the final table
non_na_counts <- rowSums(!is.na(filtered_counts))
samples_with_zero <- sum(non_na_counts == 0)
samples_with_one <- sum(non_na_counts == 1)
samples_with_more <- sum(non_na_counts > 1)

message("Sample summary: ", samples_with_zero, " samples have no non-NA values, ", 
        samples_with_one, " have exactly 1 non-NA value, ", 
        samples_with_more, " have more than 1 non-NA value.")
