...
def sort_tsv_file(input_file):
    ...
    try:
        ...
        taxa_counts = df.apply(lambda col: (col != 0).sum() if col.dtype in ['int64', 'float64'] else col.notna().sum(), axis=0)
        sorted_taxa = taxa_counts.sort_values(ascending=False).index
        df = df[sorted_taxa]
        
        # Count non-zero/non-null values in each row
        count_col = df.apply(lambda row: (row != 0).sum() if row.dtype in ['int64', 'float64'] else row.notna().sum(), axis=1)
